...
def predict_random_forest(model_bundle, filename):
    """
    Args:
    ...
    try:
        ...
        for col in numeric_columns:
            if col not in df.columns:
                df[col] = 0
                print(f"[Random Forest] Coluna numérica ausente '{col}' - preenchida com 0")
        
        for col in categorical_columns:
            if col not in df.columns:
                df[col] = "unknown"
                print(f"[Random Forest] Coluna categórica ausente '{col}' - preenchida com 'unknown'")
        ...
        print(f"[Random Forest] Erro na predição: {e}")
        import traceback
        traceback.print_exc()
        raise
